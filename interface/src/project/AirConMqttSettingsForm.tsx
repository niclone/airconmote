...
import { AirConMqttSettings } from "./types";
import { AIRCON_MQTT_SETTINGS_VALIDATOR } from "./validators";

const AirConMqttSettingsForm: FC = () => {
  const [fieldErrors, setFieldErrors] = useState<ValidateFieldsError>();
  const {
    loadData, saveData, saving, setData, data, errorMessage
  } = useRest<AirConMqttSettings>({ read: AirConApi.readBrokerSettings, update: AirConApi.updateBrokerSettings });
  ...
  const content = () => {
    ...
    const validateAndSubmit = async () => {
      try {
        ...
        await validate(AIRCON_MQTT_SETTINGS_VALIDATOR, data);
        saveData();
      } catch (errors: any) {
...
export default AirConMqttSettingsForm;
